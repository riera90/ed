...
void ed::introducirDatos(ed::Monomio &m)
{
    m.leerMonomio();
}
...
void ed::plotMonomio(ed::Monomio &m)
{
    float max, min;
    std::cout << "Introduzca el minimo y el maximo valor de x a representar" << '\n';
    std::cout << "minimo: ";
    std::cin >> min;
    std::cout << "maximo: ";
    std::cin >> max;
    std::cout << "representando el monomio "<<m<<" desde x = "<<min<<" hasta "<<max << '\n';
    m.plot(min, max);
    // Pausa
    std::cin.ignore();
    std::cout << RESET;
